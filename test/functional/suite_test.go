...
import (
	...
	octaviawebhook "github.com/openstack-k8s-operators/octavia-operator/internal/webhook/v1beta1"
...
var _ = BeforeSuite(func() {
	...
	// Keep this in synch with OctaviaReconciler.SetupWithManager,
	...
	err = octaviawebhook.SetupOctaviaWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())

	kclient, err := kubernetes.NewForConfig(cfg)
	Expect(err).ToNot(HaveOccurred(), "failed to create kclient")

	err = (&controller.OctaviaReconciler{
		Client:  k8sManager.GetClient(),
		Scheme:  k8sManager.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(k8sManager)
	Expect(err).ToNot(HaveOccurred())
	err = (&controller.OctaviaAPIReconciler{
		Client:  k8sManager.GetClient(),
		Scheme:  k8sManager.GetScheme(),
		Kclient: kclient,
