...
var _ = Describe("OctaviaAPI controller", func() {
	...
	When("an ApplicationCredential is created for Octavia", func() {
		...
		BeforeEach(func() {
			...
			acSecretName := fmt.Sprintf("ac-%s-secret", octavia.ServiceName)
			acSecret := th.CreateSecret(
				types.NamespacedName{Namespace: namespace, Name: acSecretName},
				map[string][]byte{
	...
	When("ApplicationCredential consumer finalizer is managed", func() {
		...
		It("should move the finalizer from the old to the new secret on rotation", func() {
			...
			Eventually(func(g Gomega) {
				o := GetOctaviaAPI(octaviaAPIName)
				g.Expect(o.Status.Conditions.IsTrue(condition.ReadyCondition)).To(BeTrue())
			}, timeout, interval).Should(Succeed())
